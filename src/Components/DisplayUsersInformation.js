...
////////functions
  ...
  return (
    <section id="nav-user__display__input-container">
      ...
      <TakeUserInformation isBtnClicked={isBtnClicked} setIsBtnClicked={setIsBtnClicked}/>
        <div className="add-user-btn">
          <button onClick={e => setIsBtnClicked(!isBtnClicked)}>Add User</button>
        </div>
          ...
          <table>
            ...
            {userData.length ? userData.map((item, index) => (
              <tr className="table-row" key={index+1}>
                ...
                <td>        </td>
                <td>{item.role}</td>
                <td><button className="trash-btn-wrapper" value={index} onClick={deleteDatafromLocale}><img className="trash-btn" src={trash} alt="trash"></img></button></td>
              </tr>
